...
class TimeBased(Thread):
    ...
    def detect_TCPflood(self, packet, window):
        ...
        for stream in self.__ip_cnt_TCP:
            ...
            if len(window) >= 7:
                ...
                if value[0] > 7:
                    print("Time Based with attack : "+str(value))
                    self.text.insert(END,"Time Based Possible TCP-SYN-Flood Attack from %s --> %s --> %s\n"%(src,dst,str(pckts_sent)))
                    self.malicious.append(np.sum(value))
                else:
                    self.malicious.append(np.sum(value))
                    print("Time Based without attack : "+str(value))
    ...
    def process(self, queue):
        ...
        queue.empty()        
        messagebox.showinfo("Time Based Attack Detection","RUN TIME - Based Attack Detection : "+str(len(self.getMalicious())))
